    ...
    *****************************************************/
    ...
    foreach($items as $item){
        $InvoiceLine = $xml->createElement('cac:InvoiceLine'); $InvoiceLine = $Invoice->appendChild($InvoiceLine);
            $cbc = $xml->createElement('cbc:ID', $i); $cbc = $InvoiceLine->appendChild($cbc);// id del item
            $cbc = $xml->createElement('cbc:InvoicedQuantity', $item['cantidad']); $cbc = $InvoiceLine->appendChild($cbc); $cbc->setAttribute('unitCode', "NIU");// cantidad
            $cbc = $xml->createElement('cbc:LineExtensionAmount', $item['venta']); $cbc = $InvoiceLine->appendChild($cbc); $cbc->setAttribute('currencyID', "PEN");// valor venta con descuento
            $pricing = $xml->createElement('cac:PricingReference'); $pricing = $InvoiceLine->appendChild($pricing);// precio unitario del producto con igv
                $cac = $xml->createElement('cac:AlternativeConditionPrice'); $cac = $pricing->appendChild($cac);
                    $cbc = $xml->createElement('cbc:PriceAmount', $item['unitario']); $cbc = $cac->appendChild($cbc); $cbc->setAttribute('currencyID', "PEN");// precio unitario con igv
                    $cbc = $xml->createElement('cbc:PriceTypeCode', '01'); $cbc = $cac->appendChild($cbc);// 01 con igv, 02 operaciones no onerosas
                ...
                $taxtsubtotal = $xml->createElement('cac:TaxSubtotal'); $taxtsubtotal = $taxtotal->appendChild($taxtsubtotal);
                    ...
                    $taxtcategory = $xml->createElement('cac:TaxCategory'); $taxtcategory = $taxtsubtotal->appendChild($taxtcategory);
                        ...
                        $taxscheme = $xml->createElement('cac:TaxScheme'); $taxscheme = $taxtcategory->appendChild($taxscheme);
                            ...
                            $cbc = $xml->createElement('cbc:Name', 'IGV'); $cbc = $taxscheme->appendChild($cbc);
                            $cbc = $xml->createElement('cbc:TaxTypeCode', 'VAT'); $cbc = $taxscheme->appendChild($cbc);
            $item1 = $xml->createElement('cac:Item'); $item1 = $InvoiceLine->appendChild($item1);
                $cbc = $xml->createElement('cbc:Description', preg_replace('([^A-Za-z0-9/\s\s+])', '', $item['descripcion'])); $cbc = $item1->appendChild($cbc);
            ...
            $price = $xml->createElement('cac:Price'); $price = $InvoiceLine->appendChild($price);// precio unitario sin igv ejm 83.05
                $cbc = $xml->createElement('cbc:PriceAmount', $item['unitario']); $cbc = $price->appendChild($cbc); $cbc->setAttribute('currencyID', "PEN");
        $i++;
    }
